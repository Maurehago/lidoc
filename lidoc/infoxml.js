...
/**
 * @typedef {import('./infoschema.js').PropertyType} PropertyType
 */
...
/**
 * Entfernt das XML-Namensraum-Präfix (z.B. "_:Name" -> "Name")
...
export function parseXSD(xsdString) {
    ...
    function checkElement(elm, typeName, propID) {
        ...
        const elmRef = cleanNamespace(elm.getAttribute("ref")) || undefined;
        const elmType = cleanNamespace(elm.getAttribute("type")) || undefined;
        const elmBase = cleanNamespace(elm.getAttribute("base")) || undefined;
        ...
        switch (elm.localName) {
            ...
            case "appinfo":
                ...
                break;
            case "assert":
                // XSD 1.1 Assert-Regel zur logischen Validierung auslesen
                // parent: complexType, restriction
                // attribute: id, test, xpathDefaultNamespace, ...any
                // children: annotation

                const testCondition = elm.getAttribute("test");
                if (testCondition) {
                    // Wir erstellen ein neues Assert-Objekt
                    let assert_options = XSD.newAssert();
                    assert_options.gsid = getGSID();

                    // Da ein Assert an einem complexType hängt, ist der übergeordnete 
                    // Typname in `typeName` (bzw. newTypeName des Parents) gespeichert.
                    assert_options.object_name = typeName;
                    assert_options.test = testCondition;

                    // Falls eine Dokumentation/Annotation direkt im Assert eingebettet ist,
                    // leiten wir das Parsing temporär um, um eine Custom-Fehlermeldung zu extrahieren.
                    localData.currentAssertId = assert_options.gsid;

                    XSD.setAssert(assert_options);

                    // Falls Dokumentationen (Fehlermeldungen) im Assert vorhanden sind:
                    checkChildren = true;

                    // Nach dem Durchlaufen der Kinder räumen wir das temporäre Feld auf
                    afterCheckChildren = "assert_cleanup";
                }
                break;
            case "attribute":
            ...
            case "documentation":
                ...
                // ERWEITERUNG: Wenn die Dokumentation zu einem Assert gehört,
                // speichern wir diese als Fehlermeldung (message) ab
                if (localData.currentAssertId) {
                    XSD.setAssert({ gsid: localData.currentAssertId, message: text });
                }
            ...
            case "element":
                ...
                // Typ bestimmern
                prop_options.prop_type = elmType || elmRef || "string";
            ...
            case "union":
                ...
                //newTypeName = getGSID();
                let memberTypes = cleanNamespace(elm.getAttribute("memberTypes"));
                if (memberTypes) {
        ...
        if (checkChildren) {
            ...
            switch (afterCheckChildren) {
                ...
                case "assert_cleanup":
                    localData.currentAssertId = undefined;
                    break;
    ...
    /**
     * Hilfsfunktion zum Prüfen ob ein Property auf ein Objekt verweist
     * @param {PropertyType} propType 
     */
    function check_relation(propType) {
        if (propType.max != 0) {
            // Typ lesen
            const dataType = XSD.getDataType(propType.object_name || "");
            const ref_type = XSD.getDataType(propType.prop_type || "");

            // Wenn Typ ein Objekt Typ ist
            if (dataType && ref_type && ref_type.art == "object") {
                const object_props = Array.isArray(dataType.id) ? dataType.id : [dataType.id || ""];
                const ref_props = Array.isArray(ref_type.id) ? ref_type.id : [ref_type.id || ""];
                XSD.setRef({ name: propType.name, object_name: propType.object_name, object_props, ref_name: ref_type.name, ref_props });
            }
        }
    }

    // Referenzen prüfen
    XSD.properties.forEach(check_relation);
